...
import { list_staff_url, get_users_url } from '../../app.constants';
...
export class StafflistingComponent implements OnInit {
  ...
  filterusers() {
    if (this.searchForm.valid) {
      const search_payload = {
        'email': this.searchForm.value['search_value']
      };
  ...
  fetchUsers() {
    ...
    this.loadingService.showloading();
    this.administrationService.getrecords(get_users_url, search_payload).subscribe((res) => {
      if (res) {
        this.records = res;
        this.loadingService.hideloading();
      }
