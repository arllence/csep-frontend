...
export class TokenInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if(request.url.search(exemptappconfig) !== -1 || request.url.search(exemptlogin) !== -1 || request.url.search(exemptsignup) !== -1 || request.url.search(exemptverifyemail) !== -1 || request.url.search(exemptresendotp) !== -1 || request.url.search(exemptpassreset) !== -1 || request.url.search(exemptsendresetlink) !== -1 || request.url.search(exemptresetlink) !== -1){
    ...
    .pipe(catchError((err: any) => {
      ...
      return new Observable<HttpEvent<any>>();
        ...
        request = request.clone({
            setHeaders: {
              Authorization: 'Bearer '+ localStorage.getItem('ien_token'),
            }
          });
          return next.handle(request)
    .pipe(catchError((err: any) => {
