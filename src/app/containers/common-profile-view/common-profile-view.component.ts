...
export class CommonProfileViewComponent implements OnInit {
  ...
  handleFileupload(e) {
    ...
    formData.append('documentType', 'MEMBERSHIP');

    this.administrationService.postrecord(upload_document_url, formData).subscribe((res) => {
      if (res) {
        this.certificate_id = res['doc_id'];
        // console.log(this.picture_link);
        this.toastService.showToastNotification('success', 'Successfully Uploaded', '');
      } else {
  ...
  edit_association(id,name,role, expiration, certificate_id){
  ...
  save_association(){
    if (this.AddAssociationForm.valid) {
      ...
      payload['certificate'] = this.certificate_id;
